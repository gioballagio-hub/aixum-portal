...
import { Users, Search, Mail, Building2, UserPlus, MoreVertical, Shield, ShieldAlert, Loader2 } from 'lucide-react';
...
const AdminClients: React.FC = () => {
  ...
  const updateUserRole = async (userId: string, currentRole: string) => {
    ...
    if (!confirm(`Vuoi davvero cambiare il ruolo di questo utente in ${newRole.toUpperCase()}?`)) return;

    setUpdatingId(userId);
    
    // Aggiornamento ottimistico per velocità percepita
      ...
      <div className="grid gap-6">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 text-gray-500 gap-4">
            ...
            <p className="font-bold tracking-widest uppercase text-xs">Caricamento profili...</p>
          </div>
          ...
          <div key={profile.id} className="glass-card gold-border p-8 rounded-[32px] flex flex-col md:flex-row items-center justify-between gap-8 hover:bg-white/[0.03] transition-all">
            <div className="flex items-center gap-6 w-full md:w-auto">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3">
                  ...
                  {profile.role === 'admin' ? (
                    <span className="px-2 py-0.5 rounded-md bg-gold-primary/10 border border-gold-primary/20 text-gold-primary text-[10px] font-bold uppercase flex items-center gap-1">
                      <Shield size={10} /> Admin
                    </span>
                  ) : (
                    <span className="px-2 py-0.5 rounded-md bg-white/5 border border-white/10 text-gray-500 text-[10px] font-bold uppercase">Client</span>
                  )}
                </div>
            ...
            <div className="flex items-center justify-between w-full md:w-auto md:justify-end gap-10">
              ...
              <div className="flex items-center gap-3">
                <button 
                  onClick={() => updateUserRole(profile.id, profile.role)}
                  disabled={updatingId === profile.id}
                  className={`flex items-center gap-2 px-4 py-3 rounded-xl border transition-all font-bold text-[10px] uppercase tracking-widest ${
                    profile.role === 'admin' 
                    ? 'border-red-500/20 text-red-500 hover:bg-red-500/10' 
                    : 'border-gold-primary/20 text-gold-primary hover:bg-gold-primary/10'
                  } disabled:opacity-50`}
                >
                  {updatingId === profile.id ? (
                    <Loader2 size={14} className="animate-spin" />
                  ) : (
                    profile.role === 'admin' ? <ShieldAlert size={14} /> : <Shield size={14} />
                  )}
                  {profile.role === 'admin' ? 'Rendi Client' : 'Rendi Admin'}
                </button>
